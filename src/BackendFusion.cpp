...
public:
    BackendFusion() : nh("~")
    {
        ...
        // 发布 全局地图 / 优化的里程计、轨迹 / 角点地图、面点地图、原始点云
    ...
    {
        if (slide_window_width < 1)
            return;
        if (keyframe_idx.size() < slide_window_width)
            return;
        ...
        for (int iterCount = 0; iterCount < 1; ++iterCount)
        {
            ...
            // eigen to double
            // 添加优化变量  lidar parameters  和  add IMU parameters
        ...
        for (int i = keyframe_idx[keyframe_idx.size() - slide_window_width]; i <= keyframe_idx.back(); ++i)
        {
            ...
            Eigen::Quaterniond dq = Eigen::Quaterniond(tmpQuat[i - keyframe_idx[keyframe_idx.size() - slide_window_width]][0],
                                                       ...
                                                       tmpQuat[i - keyframe_idx[keyframe_idx.size() - slide_window_width]][3])
                                        .normalized()
                                        .inverse() *
    ...
    void saveKeyFramesAndFactors()
    {
        ...
        {
            //TODO：：optimizeSlidingWindowWithLandMark
        ...
        else if (pose_cloud_frame->points.size() > slide_window_width)
        {
            ...
            tmpTrans = Eigen::Quaterniond(pose_info_each_frame->points[jj - 1].qw,
                                          ...
                                          pose_info_each_frame->points[jj - 1].qz)
                           .inverse() *
                       tmpTrans;
    ...
    void performLoopClosure()
    {
        ...
        {
            // 回环检测！ 使用RS进行回环！
            // latest_key_frames_ds  以及  his_key_frames_ds！！
