...
private:
    ...
    Eigen::Quaterniond q_lb; //  laser and base_link
...
public:
    ...
    PointType undistortion(PointType pt, const Eigen::Quaterniond quat)
    {
        ...
        q_si = q_lb * q_si * q_lb.inverse(); //  laser-->base_link-->laser
    ...
    void cloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg)
    {
        ...
        for (int i = 0; i < cloudSize; i++)
        {
            ...
            // 在这里进行去畸变
            point_undis = undistortion(point, qIMU); //  只矫正方向变化，不矫正平移
        ...
        qIMU = Eigen::Quaterniond::Identity(); //  处理完一帧后，方向角重置
...
//  进来数据，利用imu加速度积分，进行角度矫正（平移不处理）
//  然后提取特征点（手动计算ring 和 time ， 提取 角点和 面点）
